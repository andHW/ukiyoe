...
import TakenTilesPanel, { type HistoryEntry } from "./components/TakenTilesPanel";
import BoardCodeBar from "./components/BoardCodeBar";
...
export default function App() {
  ...
  useEffect(() => {
    if (gameMode === "vs-ai" && state.currentPlayer === "p2" && !state.isGameOver && !isThinking) {
      const timer = setTimeout(() => requestMove(state, difficulty), 400);
  ...
  return (
    <AppRoot>
      ...
      <Box sx={{ position: "relative", width: "100%", maxWidth: 1000, display: "flex", justifyContent: "center" }}>
        <CenterColumn>
          <GameBoard
            state={state}
